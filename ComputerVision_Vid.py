...
def get_vals(corners):
    ...
    # This vector contains x, y, z distances of marker from camera
    t_vec = tvecs[0][0]
...
if __name__ == '__main__':
    ...
    while True:
        if state == 0:
            # State 0 for continuous video search for Aruco marker
            state = state0(state)
                
        if state == 1:
            # Get state1 start time
            start_time = time()
            # State 1 for still image capture to localize Aruco marker
            distance, angle_deg, angle_rad = state1()

            # Print the runtime of the program
            print("state1 runtime: ", time() - start_time, "seconds")
